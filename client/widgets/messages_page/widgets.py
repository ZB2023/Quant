...
from PySide6.QtCore import (
    ...
    QBuffer, QIODevice, QPoint, QUrl, QPointF, QRectF, QRect
)
...
class ModernAvatar(QWidget):
    ...
    def set_data(self, t, d=None):
        ...
        if self.ld and self.ld.isRunning(): self.ld.terminate()
        if not d: 
            self.pm=None
            self.mv=None
            self.update()
    ...
    def _L64(self, s):
        try: _,e=s.split(',',1); self._Lb(base64.b64decode(e))
        except: pass
...
class ActionMorphButton(QAbstractButton):
    ...
    def enterEvent(self,e): self.am.setEndValue(1.0); self.am.start(); super().enterEvent(e)
    def leaveEvent(self,e): self.am.setEndValue(0.0); self.am.start(); super().leaveEvent(e)
    def animate_send(self):
        if self.mode=='send': self.asend.stop(); self.asend.setStartValue(0.0); self.asend.setEndValue(1.0); self.asend.start()
    def set_theme(self, d): self.is_dark=d; self.update()
    def paintEvent(self,e):
        p=QPainter(self)
        p.setRenderHint(QPainter.Antialiasing)
        c=self.rect().center()
        ...
        if self.mode=='attach':
            ...
            f=self._s
            p.translate(c)
            if f>0: p.translate(30*f,-30*f); p.setOpacity(1-f)
            pa=QPainterPath()
            pa.moveTo(-4,-3)
            pa.lineTo(5,1)
            pa.lineTo(-4,5)
...
class ChatListItem(QWidget):
    ...
    def __init__(self, data, is_collapsed=False):
        ...
        if ts:
            try: dt = date_parser.parse(ts); self.date_lbl.setText(dt.strftime("%d.%m.%Y"))
            except: pass
        ...
        if ATTACHMENT_SPLITTER in msg:
            ...
            t = p[0].strip()
            if t: msg = t
            elif any('cmd://image' in x for x in p): msg = "🖼️ Изображение"
            elif any('cmd://file' in x for x in p): msg = "📄 Документ"
            else: msg = "Вложение"
        elif "cmd://image" in msg: msg = "🖼️ Изображение"
        elif "cmd://file" in msg: msg = "📄 Документ"
...
class ChatBubble(QFrame):
    ...
    def update_bubble_theme(self, is_dark):
        ...
        if self.lbl_text: self.lbl_text.setStyleSheet(f"color: {tc}; border:none; background:transparent;")
        self.t_lbl.setStyleSheet(f"font-size:10px; color:{mc}; background:transparent;")
        if self.s_lbl: self.s_lbl.setStyleSheet(f"font-size:10px; font-weight:bold; margin-left:4px; color:{sc}; background:transparent;")
